...
@section('title', 'Nuevo Tamaño de Vehículo')

@section('content_header')
    <h1><i class="fas fa-ruler"></i> Nuevo Tamaño de Vehículo</h1>
@endsection

@section('content')
<div class="card">
    <div class="card-body">
        <form action="{{ route('tamanos-vehiculo.store') }}" method="POST">
            @csrf
            
            <div class="form-group">
                <label for="nombre">Nombre *</label>
                <input type="text" name="nombre" id="nombre" class="form-control @error('nombre') is-invalid @enderror" 
                       value="{{ old('nombre') }}" required placeholder="Ej: Pequeño, Mediano, Grande">
                @error('nombre')
            ...
            <div class="form-group">
                ...
                <textarea name="descripcion" id="descripcion" class="form-control @error('descripcion') is-invalid @enderror" 
                          rows="3" placeholder="Descripción opcional del tamaño">{{ old('descripcion') }}</textarea>
                @error('descripcion')
                    <span class="invalid-feedback">{{ $message }}</span>
                @enderror
            </div>
                ...
                <div class="col-md-6">
                    <div class="form-group">
                        ...
                        <input type="number" step="0.01" name="capacidad_min" id="capacidad_min" 
                               ...
                               value="{{ old('capacidad_min') }}" placeholder="0.00">
                        @error('capacidad_min')
                            <span class="invalid-feedback">{{ $message }}</span>
                        @enderror
                    </div>
                ...
                <div class="col-md-6">
                    <div class="form-group">
                        ...
                        <input type="number" step="0.01" name="capacidad_max" id="capacidad_max" 
                               ...
                               value="{{ old('capacidad_max') }}" placeholder="0.00">
                        @error('capacidad_max')
                            <span class="invalid-feedback">{{ $message }}</span>
                        @enderror
                    </div>
